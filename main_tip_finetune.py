...
if __name__ == '__main__':
    ...
    parser.add_argument('--unseen_pt_inj', action='store_true', help='Enable EZ-HOI UTPL for unseen HOI classes')

    # ContextHOI SSCE (semantic-guided context exploration)
    parser.add_argument('--use_ssce', action='store_true', help='Enable ContextHOI SSCE to enhance CLIP local features')
    parser.add_argument('--ssce_topk_ins', type=int, default=30, help='Top-k spatial tokens for instance exploration')
    parser.add_argument('--ssce_topk_int', type=int, default=30, help='Top-k spatial tokens for interaction exploration')
    parser.add_argument('--ssce_mlp_hidden', type=int, default=512, help='Hidden dim of SSCE MLPs')
    parser.add_argument('--ssce_temperature', type=float, default=1.0, help='Softmax temperature for SSCE token scoring')

    # Hybrid-HOI: Hybrid Learning (3.2) with Hybrid decay scheme
    parser.add_argument('--use_hybrid', action='store_true',
                        help='Enable Hybrid Learning: add teacher-based pseudo-label loss on unlabeled HO pairs')
    parser.add_argument('--hybrid_teacher_path', type=str, default='',
                        help='Path to a pretrained checkpoint used as the frozen teacher for Hybrid Learning')
    parser.add_argument('--hybrid_sigma', type=float, default=1.0,
                        help='Initial weight sigma for pseudo-label loss (lambda(0)=sigma)')
    parser.add_argument('--hybrid_tstop', type=int, default=10,
                        help='Epoch to stop pseudo-label loss (lambda(t)=0 for t>=tstop)')
